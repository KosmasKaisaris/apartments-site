...
export function HomeClient(props: { a: Apartment; b: Apartment }) {
  const { dict, locale } = useI18n();
  const aData = getApartmentLocaleData(props.a, locale);
  const bData = getApartmentLocaleData(props.b, locale);
        ...
        <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-5 sm:py-14">
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-2">
            <SplitPanel
              href={`/${props.a.slug}`}
              title={aData.name}
              subtitle={aData.tagline}
              meta={aData.locationLine}
              imageSrc={props.a.heroImage.src}
              imageAlt={props.a.heroImage.alt}
            />
            <SplitPanel
              href={`/${props.b.slug}`}
              title={bData.name}
              subtitle={bData.tagline}
              meta={bData.locationLine}
              imageSrc={props.b.heroImage.src}
              imageAlt={props.b.heroImage.alt}
            />
          </div>
